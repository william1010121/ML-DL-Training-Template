...
| preprocess | `run(out, params, source)` → 寫資料 + `manifest.json` |
| model | `build(params, manifest) -> nn.Module` |
| algorithm | `train(model, data, out, params, seed, smoke)`：每 epoch append `metrics.jsonl`；寫 `ckpt_last.pt`，有就 resume；結束寫 `ckpt.pt`；`smoke` 時 1 epoch 少量樣本 |
| evaluate | `score(model, data, params) -> dict[str, float]`；model 由 core 依 train 的 `meta.json` 重建並載入 `ckpt.pt` |

- 變體只能 import 標準庫、pyproject 已列套件、同軸 `_common.py`、`protocols`。禁止跨軸、禁止 import core。
- 邊界有兩種：source、preprocess、train、evaluate 四個 stage 之間只透過磁碟目錄格式溝通；train stage 內的 model→algorithm 與 evaluate stage 內的 model→score 是唯一允許的 Python 物件邊界。物件一律是由 core 建立並傳入的通用 `nn.Module`；algorithm 與 evaluate 不得 import 具體 model 變體，也不得依 model 名稱寫分支。
...
- 目錄 `cache/<stage>/<name>-<key>`，`key = sha256(canonical_json({stage, src, params, up, cache_version}))[:12]`；src = 變體檔全文 + `_common.py` 全文（train 為 model+algorithm 兩者串接）；up = 上游目錄名；train 的 name 為 `<model>+<algorithm>`。
...
- exp 是完整 YAML：五軸各 `{name, params}` + `seed`；可 `extends: base.yml`（同目錄、一層、深合併）。Pydantic `extra="forbid"`，name 必須對應存在的檔案。
- `python run.py configs/<line>/exp-NNN.yml [--smoke]`：展開、驗證、記 commit/dirty、跑四 stage、寫 runs/。`--smoke` 輸出到 `cache/_smoke/`，不記錄。stdout 只印 run 路徑，其餘走 stderr。
...
import 規則、config 可展開且 name 可解析、`runs/` 的 resolved.yml 與現在展開結果一致、cache key 對 src/params/up 任一變動都改變、ACCEPTS 正反例、模型能力 Protocol 正反例、train `meta.json` 可重建模型、evaluate 拒絕非有限指標並寫出 canonical `metrics.json`、`tests/smoke.yml` 以 `--smoke` 在 CPU 60 秒內跑通、每個 goal.md 四段齊全。
